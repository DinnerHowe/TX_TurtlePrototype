...
class go_task():
        ...
        if state_list != []:
            for i in state_list:
                ...
                if i.status in self.active_state.keys():
                    rospy.loginfo('task inprogress plz wait')
                    pass
    ...
    def OdomCB(self, data):
        with self.locker:
            ...
            if len(self.tasks):
                ...
                global CurrentGoal
                CurrentGoal = marker.pose.position
                task_goal = self.find_goal(pose.position, CurrentGoal)
                self.move_base.send_goal(task_goal)
    ...
    def find_goal(self, initposition, taskposition):
        ...
        try:
            task_goal.target_pose.header.frame_id = self.marker_point.header.frame_id
        except:
            task_goal.target_pose.header.frame_id = 'map'
        task_goal.target_pose.pose.position = taskposition
        task_goal.target_pose.pose.orientation.x = gqx
        task_goal.target_pose.pose.orientation.y = gqy
        task_goal.target_pose.pose.orientation.z = gqz
        task_goal.target_pose.pose.orientation.w = gqw
        return task_goal
